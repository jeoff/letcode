...
public class SalesforceRest {
    ...
    static final String USERNAME     = "qingyun.admin@veeva.qa.em2";
    static final String PASSWORD     = "Pub123456Un7sUb5qsXl3eriuFNTBbJhzb";
    static final String LOGINURL     = "https://login.salesforce.com";
    static final String GRANTSERVICE = "/services/oauth2/token?grant_type=password";
    static final String CLIENTID     = "3MVG9vtcvGoeH2biAXbjYQCWa_qQq3TD6mJehABpW051MT2xPIMFx15sBJX3nliTW1.R9dWaCGtNSgVYod04s";
    static final String CLIENTSECRET = "4991F5EAB19E9CD6099FC26F3EEC778B86C4B8B03D200B81842512752929F298";
    ...
    public static void queryLeads() {
        ...
        try {
            ...
            //String uri = baseUri + "/query?q=+Id+,+FirstName+,+LastName+,+Company+From+Lead+WHERE+FirstName+!=+''+Limit+5";
            ...
            if (statusCode == 200) {
                ...
                try {
                    ...
                    for (int i = 0; i < j.length(); i++){
                        JSONObject jsobject = json.getJSONArray("records").getJSONObject(i);
                        leadId = jsobject.getString("Id");
                        leadFirstName = jsobject.getString("FirstName");
                        leadLastName = jsobject.getString("LastName");
                        leadCompany = jsobject.getString("Company");
    ...
    // Query Leads using REST HttpGet
    public static void queryEventsMembers() {
        System.out.println("\n_______________ Event Member Query _______________");
        try {

            //Set up the HTTP objects needed to make the request.
            HttpClient httpClient = HttpClientBuilder.create().build();

            String uri = baseUri + "/query?q=Select+Id+,+Name+,+(Select+Id+,+Attendee_Type_vod__c+,+Attendee_vod__c+from+Event_Attendee_vod__r)+from+Medical_Event_vod__c+Where+Id+='a0p3i000000BxVOAA0'";
            System.out.println("Query URL: " + uri);
            HttpGet httpGet = new HttpGet(uri);
            System.out.println("oauthHeader2: " + oauthHeader);
            httpGet.addHeader(oauthHeader);
            ...
            // Process the resultAttendee_Type_vod__c
            int statusCode = response.getStatusLine().getStatusCode();
            if (statusCode == 200) {
                String response_string = EntityUtils.toString(response.getEntity());
                try {
                    ...
                    System.out.println("JSON result of Query:\n" + json.toString(1));
                    JSONArray j = json.getJSONArray("records");
                    JSONObject eventJs = (JSONObject)j.get(0);
                    String eventId = eventJs.getString("Id");
                    String eventName = eventJs.getString("Name");
                    System.out.println("Event name is, " +eventName);
                    JSONArray member = eventJs.getJSONObject("Event_Attendee_vod__r").getJSONArray("records");
                    for (int i = 0; i < member.length(); i++){
                        String attendId = member.getJSONObject(i).getString("Id");
                        String attendeeType = member.getJSONObject(i).getString("Attendee_Type_vod__c");
                        String attendeeName = member.getJSONObject(i).getString("Attendee_vod__c");
                        System.out.println("Attendee record is: " + attendId + " , attendeeName " + attendeeName);
                    }
                } catch (JSONException je) {
                    je.printStackTrace();
                }
